...
- **Gate de modelo/fase:** antes de iniciar qualquer fase, o skill **lê o modelo ativo** e emite um aviso de capacidade conforme a fase. Modelo default de referência do kit: `ollama-cloud` + `glm-5.3` (também configurado como default no sandbox Linux por `scripts/setup-linux-sandbox.sh`). Mapa de adequação:
  ...
  - **APPLY (mudanças aprovadas em perfis/cofres/sudo)**: exige modelo de maior confiança para aderência a instrução e tool-calling. Default recomendado `glm-5.3` (validado em produção/sandbox); alternativas por ranking: `nemotron-3-ultra`, `kimi-k3`, `qwen3.5:397b`. O skill **avisa** se o modelo ativo for Flash (ou outro de teto menor) antes de executar APPLY, e nunca aplica sem aprovação humana visível dos diffs. **Alternativas de outros provedores** (quando disponíveis/autenticadas no perfil — o kit valida acesso antes de sugerir):
  - **Anthropic** (via `ANTHROPIC_API_KEY`): `claude-sonnet-4` como equilíbrio robusto para APPLY; `claude-opus` para os passos de mais alto risco (resolver cofre / tocar `sudo` / rever diff de perfil). Referência de família: `anthropic/claude-sonnet-4`.
  - **OpenAI/OpenRouter** (via `OPENROUTER_API_KEY`): `gpt-5.x` (ex.: `gpt-5.4` ou `o3`) para tool-calling de APPLY; preferir a variante com raciocínio habilitado para aderência a instrução. Via OpenRouter, também dá para chamar modelos de terceiros no mesmo fluxo.
  - **Gemini** (via `GEMINI_API_KEY`): `gemini-3-flash`/`gemini-3.6-flash` são alternativas econômicas; `gemini-3-pro` para passos críticos.
  - Modelo padrão do kit mantém `glm-5.3`; os demais são **fallbacks opcionais** que requerem crédito/chave e validação de acesso antes do uso.
